...
- ユーザーに表示する最終文言は原則フロントエンドで管理する
- 成功時のAPIレスポンスに「ログイン成功」などの表示用メッセージを無理に含めない
- 成功時は、画面遷移や表示に必要なデータを返すことを優先する
- バックエンドはHTTPステータス、エラー種別、エラーコード、バリデーション詳細を返す
- フロントエンドはAPIのエラーコードやステータスをもとに、画面ごとの日本語文言へ変換する
- APIのmessageをそのまま画面に表示する前提の実装は避ける
- 認証失敗では、メール存在有無などを推測されないようにエラー理由を分けすぎない
- 例外的にバックエンドがmessageを返す場合も、最終的な表示文言はフロントエンドで決める
