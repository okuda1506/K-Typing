...
export function TypingPage() {
    ...
    const isAnswering =
        isCountingDown || answerState !== 'idle' || isResultLoading;
    ...
    const correctTypedCount = judgedChars.filter(
        (char) => char.state === 'correct',
    ).length;
    const shouldShowStatus =
        answerState === 'idle' && hasInput && correctTypedCount > 0;
    const statusText =
    ...
    const answerMessage =
        answerState === 'correct' ? 'その調子です！' : '後で復習しましょう';
    ...
    useEffect(() => {
        countdownTimersRef.current = [
            ...
            window.setTimeout(
                () => setCountdownValue(1),
                COUNTDOWN_STEP_DELAY_MS * 2,
            ),
            window.setTimeout(
                () => setCountdownValue(null),
                COUNTDOWN_STEP_DELAY_MS * 3,
            ),
        ];
    ...
    function advanceQuestion(
        nextAccuracies: number[],
        nextMistakeCounts: number[],
    ) {
        if (isLastQuestion) {
                ...
                resultTimerRef.current = window.setTimeout(() => {
                    navigate('/sessions/mock-session/result', {
                        state: {
                            lessonId,
                            accuracy:
                                nextAccuracies.reduce(
                                    (total, accuracy) => total + accuracy,
                                    0,
                                ) / nextAccuracies.length,
                            mistakeCount: nextMistakeCounts.reduce(
    ...
    function submitAnswer() {
        ...
        const isCorrect =
            normalizeKoreanInput(inputText) ===
            normalizeKoreanInput(currentQuestion.koreanText);
    ...
    return (
        <section className="page-card typing-page">
            ...
            {isResultLoading ? (
                <div
                    className="result-loading-screen"
                    aria-live="polite"
                    aria-busy="true"
                >
                ...
                <div
                    className={`answer-result ${answerState}`}
                    aria-live="polite"
                >
